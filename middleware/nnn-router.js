...
export default (options = {}) => {
  ...
  return async (req, res, next) => {
    ...
    const middlewareSort = sortedPaths
      .filter(
        a => a[0].slice(a[0].lastIndexOf('/') + 1).slice(0, 'middleware'.length) === 'middleware'
      )
      .concat(
        sortedPaths.filter(
          a => a[0].slice(a[0].lastIndexOf('/') + 1).slice(0, 'middleware'.length) !== 'middleware'
    ...
    for (let i = 0; i < middlewareSort.length; i++) {
      ...
      if (typeof handler.middleware === 'object') {
        temporary[method](
          routePath,
          ...Object.values(handler.middleware).filter(v => typeof v === 'function')
        )
      } else if (typeof handler.middleware === 'function') {
        temporary[method](routePath, handler.middleware)
      }

      if (typeof handler === 'function') {
        temporary[method](routePath, handler)
      } else if (typeof handler.default === 'function') {
        temporary[method](routePath, handler.default)
      }
    }
